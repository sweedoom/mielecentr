...
| Веб-админка | ✅ `admin/`, вход по паролю — пароль проверяется на сервере Supabase |
...
| `admin.supabase` | Project URL + anon public ключ (ключ публичный — это нормально) |
| `leads.mode` | `supabase` — заявка уходит в хранилище напрямую из браузера |
| `policyDate` | Дата публикации политики |

---

## 6. Заявки и веб-админка
...
- Смена пароля админки: `UPDATE private.admin_auth SET pw='новый' WHERE site='mielecentr';`
  (SQL-редактор Supabase).

**Админка:** https://www.miele-technika.ru/admin/ — вход по паролю
(проверка на сервере, не в HTML). Внутри нейтральная панель заявок
(фильтры, статусы, комментарии, CSV, статистика). Админка этого сайта видит
**только заявки этого сайта** — изоляция на стороне сервера (rpc `admin_leads_site`).
...
| Посмотреть заявки | Веб-админка: https://www.miele-technika.ru/admin/ |
...
- ✅ Все формы шлют заявки в хранилище
- ✅ Админка с проверкой пароля на сервере
